...
exports.deleteCustomerWishlists = function(parameters)
{
	var customerNo = parameters.customerNo;
	var customer : Customer = CustomerMgr.getCustomerByCustomerNumber(customerNo);
	var customerWishList : Collection = null;
	
	customerWishList = ProductListMgr.getProductLists(customer , dw.customer.ProductList.TYPE_WISH_LIST);			
	if(!empty(customerWishList))
	{
		var wishlistIterator : Iterator = customerWishList.iterator();
		while(wishlistIterator.hasNext()){
			Transaction.wrap(function () {
                ProductListMgr.removeProductList(wishlistIterator.next());
            });			
		}
	}	
}
